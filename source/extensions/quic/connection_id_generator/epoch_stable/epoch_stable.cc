...
Context::create(Server::Configuration::FactoryContext& context) {
  if (context.serverFactoryContext().options().concurrency() > EPOCH_STABLE_MAX_WORKERS) {
    return absl::InvalidArgumentError(
        fmt::format("envoy.quic.connection_id_generator.epoch_stable cannot be used with a "
                    "concurrency greater than {}",
                    EPOCH_STABLE_MAX_WORKERS));
  }
  return std::unique_ptr<Context>(
      new Context(context.serverFactoryContext().options().concurrency()));
}
...
absl::StatusOr<std::unique_ptr<Context>> Context::create(Server::Configuration::FactoryContext&) {
  return absl::InvalidArgumentError(
      "envoy.quic.connection_id_generator.epoch_stable is not supported on this platform");
}

#endif // ENVOY_EPOCH_STABLE_EBPF_SUPPORTED

QuicConnectionIdWorkerSelector Factory::getCompatibleConnectionIdWorkerSelector() {
...
EnvoyQuicConnectionIdGeneratorFactoryPtr Context::createQuicConnectionIdGeneratorFactory(
  ...
  return std::make_unique<Factory>(concurrency_, listen_socket_factory);
}
