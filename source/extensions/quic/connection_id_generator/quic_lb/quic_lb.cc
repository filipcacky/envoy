...
Factory::Factory(
    ...
    : config_(config), concurrency_(concurrency) {}
...
Factory::create(const envoy::extensions::quic::connection_id_generator::quic_lb::v3::Config& config,
  ...
  std::unique_ptr<Factory> ret(
      new Factory(config, context.serverFactoryContext().options().concurrency()));
...
Factory::createCompatibleLinuxBpfSocketOption() {
  ...
  filter_ = {
      ...
      {0x50, 0, 0, 0000000000},  {0x35, 10, 0, concurrency_}, {0x16, 0, 0, 0000000000},
      ...
      {0x20, 0, 0, 0xfffff020},  {0x94, 0, 0, concurrency_},  {0x16, 0, 0, 0000000000},
  };
